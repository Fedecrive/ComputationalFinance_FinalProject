...
for i = 1:length(CallexpDates)
    ...
    if ~isnan(CallPrices(i)) && ~(mon < 0.005 && mon > -0.005) && year(CallexpDates(i))~=2017 && year(CallexpDates(i))~=2018
        ...
        pricesModelEtaUp =[ pricesModelEtaup, PriceCallOption(CallStrikes(i),fwd_prices(idx),disc_i,@(u)phi_fun(u,ttm_i,eta+0.01,kappa,sigma),M,dz,a)];
...
for i = 1:length(PutexpDates)
    ...
    if ~isnan(PutPrices(i)) && (mon < 0.005 && mon > -0.005) && year(PutexpDates(i))~=2017 && year(PutexpDates(i))~=2018
        disc_i = disc(idx+1);                  % Discount factor for this maturity
        ttm_i  = yearfrac(t0, dates(idx), 3);  % Maturity in anni
        pricesModelEtaUp =[ pricesModelEtaup, PriceCallOption(PutStrikes(i),fwd_prices(idx),disc_i,@(u)phi_fun(u,ttm_i,eta+0.01,kappa,sigma),M,dz,a)-disc_i*(fwd_prices(idx)-PutStrikes(i))];
